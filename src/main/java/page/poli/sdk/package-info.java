 ...
 * Two facades sit on the same client: a blocking surface ({@code client.render()},
 * {@code client.documents()}) and a {@link java.util.concurrent.CompletableFuture}-based async
 * surface ({@code client.renderAsync()}, {@code client.documentsAsync()}). Both share one HTTP
 * connection pool, one retry policy, and one exception hierarchy.
 *
