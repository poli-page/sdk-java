...
| `poli-page/sdk-csharp` | Core | NuGet | `PoliPage` |
| `poli-page/sdk-java` | Core | Maven Central | `page.poli:sdk` |
...
| Order | Deliverable | Why this slot |
|---|---|---|
| 5.1 | `poli-page/sdk-csharp` | Foundation. Targets `net8.0` + `net10.0` (Microsoft's two current LTS). Idiomatic `*Async` surface, `Microsoft.Extensions.Logging` + `Microsoft.Extensions.DependencyInjection` integration, `HttpClient` reuse via `IHttpClientFactory`. |
| 5.2 | Recipes inside `sdk-csharp/samples/`: ASP.NET Core minimal API, Worker Service, Azure Function (in-process + isolated), AWS Lambda | Covers the four .NET server deployment shapes Camille / Tomasz personas overlap with. Per spec §9, no Blazor WebAssembly recipes — API keys are server-side secrets. |

**Estimated effort**: 4-6 days.

The .NET ecosystem is converged enough on `Microsoft.Extensions.*` that
a single SDK package serves ASP.NET Core, Worker Services, Azure
Functions, and AWS Lambda equally well — no framework-integration
sub-packages needed (similar to Go's positioning).

### Phase 6 — Java (P6)

| Order | Deliverable | Why this slot |
|---|---|---|
| 6.1 | `poli-page/sdk-java` | Foundation. Targets Java 17 LTS as the floor; CI matrix Java 17 + 21 + current. Built on `java.net.http.HttpClient` (built-in since Java 11). Both blocking and `CompletableFuture<T>` async surfaces shipped in the same artifact. SLF4J for logging, Jackson for JSON. |
| 6.2 | Recipes inside `sdk-java/examples/`: Spring Boot 3 controller, Quarkus REST resource, Micronaut controller, AWS Lambda (Java runtime), plain `main`. | Same single-artifact serves Spring, Quarkus, Micronaut — Java's DI frameworks all consume idiomatic POJO beans. Spring Boot integration is the highest-volume slice. |

**Estimated effort**: 5-7 days.

Java is positioned last because the enterprise Java audience moves
slowest on new vendor SDKs — letting the spec mature through five
prior phases means the Java port lands on a maximally stable contract.
Kotlin / Scala consumers use the same JAR with their own idioms (no
separate publishing target).
...
| P5 (`sdk-csharp` + samples) | 4-6 days |
| P6 (`sdk-java` + examples) | 5-7 days |
| **Total** | **~5-6 weeks** |
...
- **v1.3 — 2026-05-29** — Added Phase 5 (`sdk-csharp`) and Phase 6
  (`sdk-java`) to cover the .NET and JVM ecosystems. Repo target bumped
  from 10 to 12. .NET is positioned mid-roadmap (after PHP) because the
  ecosystem is converged enough that no framework-integration
  sub-packages are required — a single package on NuGet serves
  ASP.NET Core / Worker Services / Azure Functions / AWS Lambda. Java
  is positioned last because the enterprise audience benefits most
  from a maximally stable spec; Kotlin / Scala consumers reuse the
  same JAR without separate publishing targets. Total effort estimate
  revised to ~5-6 weeks.
