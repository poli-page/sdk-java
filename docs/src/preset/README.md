...
The same sync needs to happen for every other SDK that vendors the preset (sdk-go, sdk-python, sdk-ruby, sdk-php, sdk-rust). That's the trade-off of skipping the npm-published path.
...
This SDK pre-dates Java support in the upstream preset. Until the upstream
adds it, `languages.js` was patched locally to include a `java` entry
(`displayName: 'Java'`, `exampleExt: '.java'`, `packageKind: 'maven'`).
When the upstream preset gains a `java` entry, the next sync drops this
local patch.
